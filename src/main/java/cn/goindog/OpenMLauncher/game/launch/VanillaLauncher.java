...
public class VanillaLauncher {
    ...
    private void launch(JsonObject versionJson, String gameName) throws IOException, InterruptedException {
        ...
        String gameDir = System.getProperty("oml.gameDir")
                + File.separator
                + "versions"
                + File.separator
                + gameName;
        String assetsDir = gameDir
                + File.separator
                + "assets";
        String assetIndex = versionJson.getAsJsonObject("assetIndex")
                .get("id").getAsString();
        ...
        if (versionJson.has("arguments")) {
            ...
            JsonArray game_argument = new Gson().fromJson(
    ...
    private JsonObject getUserConfig() throws IOException {
        ...
        JsonObject userConfigObj = new Gson().fromJson(
                FileUtils.readFileToString(
                        userConfig,
                        StandardCharsets.UTF_8
                ),
    ...
    private String getLaunchCommand(
        ...
        for (JsonElement element : libraries) {
            if (element.getAsJsonObject().get("downloads").getAsJsonObject().has("artifact")) {
                ...
                libs.append(assetDir.replace("assets", "libraries/" + path)).append(";");
            }
        }
        ...
        for (JsonElement element : arguments.getAsJsonArray("jvm")) {
            if (element.isJsonObject()) {
                ...
                if (element.getAsString().contains("${natives_directory}")) {
                    ...
                    JVMBuilder.append(" \"").append(element.getAsString().replace("${classpath}", libs)).append("\" ");
                } else {
